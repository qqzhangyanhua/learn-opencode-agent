...
<ChapterActionPanel
  :actionItems="[
    { title: '先读一个最短工具实现', description: '优先打开 bash.ts 或 read.ts，对照本章调用链把 Tool.define、registry 和 execute 真正串起来。' },
    { title: '继续进入会话管理', description: '下一章会把工具调用接回消息流、上下文压缩和死循环防护。', href: '/04-session-management/' },
    { title: '动手验证 ReAct 里的工具调用', description: '如果你更想看工具在推理链里的表现，直接去做 P10。', href: '/practice/p10-react-loop/' }
  ]"
/>
