...
**重要说明**：词频向量是演示用的简化方案，无法捕捉语义相似性（"整洁"和"风格"在词频向量中没有任何关联）。生产环境使用 Embedding 模型（如 Anthropic 的 `voyage-3` 或 OpenAI 的 `text-embedding-3-small`），这类模型把语义相似的文本映射到距离近的向量。
 ...
 * 生产环境请使用 Embedding 模型（如 voyage-3 / text-embedding-3-small）。
 */
...
常用 Embedding 模型：Anthropic voyage-3、OpenAI text-embedding-3-small、本地模型 all-MiniLM-L6-v2。
...
[检索到 3 个相关片段]
  ...
  片段3: 常用 Embedding 模型：Anthropic voyage-3、OpenAI text-embedding-3-small、本地模型...
...
Embedding 模型选型：
- **Anthropic voyage-3**：与 Claude 配合效果好，中英文双语优秀
