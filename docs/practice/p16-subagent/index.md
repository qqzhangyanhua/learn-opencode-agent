...
<RunCommand command="bun run p16-subagent.ts" />
  ...
  schema: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        dimension: { type: 'string', description: '分析维度：security | performance | quality' },
      },
  ...
  async run(task: string): Promise<SubAgentResult> {
    ...
    while (iterations < this.config.maxIterations) {
      ...
      if (response.stop_reason === 'end_turn' || toolUseBlocks.length === 0) {
        ...
        console.log(`  [${this.config.name}] 完成，${iterations} 轮，${this.callLog.length} 次工具调用`)
        return {
      ...
      for (const toolUse of toolUseBlocks) {
        ...
        const result = await toolDef.execute(toolUse.input as Record<string, string>)
        toolResults.push({
          type: 'tool_result',
          tool_use_id: toolUse.id,
          content: result,
...
async function orchestrate(userMessage: string): Promise<string> {
  ...
  const tasks: SubTaskSpec[] = JSON.parse(jsonMatch[0])
  console.log(`[Orchestrator] 拆解为 ${tasks.length} 个子任务：`)
  tasks.forEach(t => console.log(`  - [${t.agentType}] ${t.title}`))
  console.log()
...
- P15 的 Worker 是单次 `messages.create` 调用，这里的 SubAgent 有完整的 `while` 循环
