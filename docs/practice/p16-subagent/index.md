...
const workerVsSubAgentSyncSegments = [
  { actor: 'manager', state: 'done', label: '发出任务', weight: 1, active: false },
  { actor: 'wait', state: '', label: '等待一次性回答', weight: 2.5, active: false },
  { actor: 'subagent', state: 'done', label: '返回单段结论', weight: 1.5, active: false },
]

const workerVsSubAgentAsyncManagerSegments = [
  { actor: 'manager', state: 'done', label: '分派研究任务', weight: 1, active: false },
  { actor: 'manager', state: '', label: '继续协调其他子任务', weight: 3, active: false },
  { actor: 'manager', state: 'done', label: '接收最终结果', weight: 1, active: false },
]

const workerVsSubAgentAsyncAgentSegments = [
  { actor: 'idle', state: '', label: '', weight: 1, active: false },
  { actor: 'subagent', state: '', label: 'web_search', weight: 1.2, active: false },
  { actor: 'subagent', state: '', label: 'read_page', weight: 1.2, active: false },
  { actor: 'subagent', state: '', label: 'summarize', weight: 1.1, active: false },
...
<TaskDelegationDemo
  title="P16 Worker 与 SubAgent 的执行差异"
  sync-title="普通 Worker：单次调用后结束"
  async-title="SubAgent：独立循环 + 专属工具集"
  manager-label="Orchestrator"
  background-label="SubAgent"
  sync-note-text="普通 Worker 只能一次性给出回答，无法继续搜索、读取和修正。"
  async-note-text="SubAgent 会在自己的循环里连续调用工具，直到拿到足够证据再回传。"
  :sync-segments="workerVsSubAgentSyncSegments"
  :async-manager-segments="workerVsSubAgentAsyncManagerSegments"
  :async-agent-segments="workerVsSubAgentAsyncAgentSegments"
  idle-status="对比一次性回答和自主多轮执行的根本差异。"
/>
