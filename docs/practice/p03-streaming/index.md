...
> 开始前先看：[实践环境准备](/practice/setup)。如果你想先看浏览器侧的流式效果，也可以直接打开 [本章在线 Playground](/practice/playground/?chapter=p03-streaming)；相关限制和安全提示见 [在线运行模式](/practice/setup#online-mode)。本章对应示例文件位于 `practice/` 目录，可直接按命令运行。
...
- 如果你走在线运行模式，先打开 [本章在线 Playground](/practice/playground/?chapter=p03-streaming) 并填写 `API Key / baseURL / model`
