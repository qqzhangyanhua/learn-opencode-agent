...
<ChapterLearningGuide :audience="['已经会调模型，想先跑通第一个最小 Agent 项目的人']" />
...
<ChapterActionPanel
  :actionItems="[
    { title: '先改一个工具返回值再跑一遍', description: '把天气模拟数据改掉，再执行 `bun run p01-minimal-agent.ts`，观察模型回复如何跟着工具结果变化。' },
    { title: '继续做 P10：ReAct Loop', description: '把最小工具调用升级成显式 Thought / Action / Observation 推理链。', href: '/practice/p10-react-loop/' },
    { title: '回看理论篇第 1 章', description: '把你刚跑通的最小闭环重新映射回 Agent、LLM 和工具系统的概念骨架。', href: '/00-what-is-ai-agent/' }
  ]"
/>
