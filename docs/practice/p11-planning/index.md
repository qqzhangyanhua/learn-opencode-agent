...
<ProjectCard
  ...
  duration="60 min"
  :prerequisites="['P10']"
  :tags="['Planning', 'Plan-and-Execute', 'TypeScript', 'Anthropic SDK']"
/>
...
最直接的做法是在 prompt 里要求 JSON 格式，然后解析。Anthropic Claude 在格式遵从上表现稳定，只要 prompt 写清楚，JSON 输出的可靠性很高：
...
<RunCommand command="bun run p11-planning.ts" />
...
- 输出要具体、完整，供后续步骤使用`
    ...
    if (toolCallMatch) {
      return await this.handleToolCall(toolCallMatch[1], text)
    }
...
2. 降低 `temperature`（Claude API 默认约 1.0，规划场景可以降到 0.3-0.5）
