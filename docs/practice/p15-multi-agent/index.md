...
const orchestratorTaskTypes = [
  {
    ...
    label: '安全审查',
    targetAgent: 1,
    thought: '这个子任务只关注注入、越权和敏感信息泄露。',
    log: '分配给 Security Worker：检查攻击面与危险工具调用。',
  },
  {
    icon: 'P',
    label: '性能审查',
    targetAgent: 2,
    thought: '这里需要评估热点路径、重复计算和资源浪费。',
    log: '分配给 Performance Worker：检查复杂度、缓存与 I/O。'
  },
  {
    icon: 'C',
    label: '代码风格审查',
    targetAgent: 3,
    thought: '这部分更关注可读性、一致性和接口设计。',
    log: '分配给 Style Worker：检查命名、结构与可维护性。'
  },
]

const orchestratorAgents = [
  { name: 'Review Orchestrator', role: '拆解任务并聚合结果' },
  { name: 'Security Worker', role: '安全专项' },
  { name: 'Performance Worker', role: '性能专项' },
  { name: 'Style Worker', role: '风格专项' },
]
</script>
...
<AgentDispatchDemo
  title="P15 编排器任务拆解"
  orchestrator-name="Review Orchestrator"
  :task-types="orchestratorTaskTypes"
  :agents="orchestratorAgents"
  idle-status="选择一个代码审查维度，观察 Orchestrator 如何分派。"
/>
...
注意 Worker 用的是示例 Sonnet 型号而不是 Opus——Worker 执行的是聚焦子任务，通常更适合性价比模型；Orchestrator 则更适合承担任务拆解和聚合。这种**分层用模型**的策略在生产中非常常见，但具体型号应以你当前账号可用模型为准。
