---
title: "92 关键类型与核心抽象"
description: "关键类型与核心抽象。基于 Claude Code 源码业务流专栏，梳理关键设计决策、运行时边界与可复用工程判断。"
---
...
> 说明：当前还原包里，多处模块会引用 `src/types/message.js`，但该源文件本体没有完整落在当前 `restored-src/src/types/` 目录快照中。因此本章关于“消息模型”的部分，主要基于 [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) 与 [utils/messages.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/utils/messages.ts) 的消费侧反向归纳，不虚构缺失定义。
...
| --- | --- | --- | --- |
| 消息模型 | [utils/messages.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/utils/messages.ts)、[Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) | 把用户消息、assistant 响应、工具结果、系统事件放入统一轨迹 | 没有统一消息模型，就不会有可继续推进的 query loop |
| `ToolUseContext` | [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) | 为工具执行提供完整上下文容器 | 它是主循环、工具、技能、MCP、任务系统的公共地基 |
| `Tool` / `ToolResult` | [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) | 定义工具输入、执行、结果回填、UI 渲染与错误路径 | 它把“模型决定做事”变成“系统真正执行” |
| `TaskStateBase` + 各类任务状态 | [Task.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Task.ts)、[tasks/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/types.ts) | 把本地/远程/协同执行单元统一成可托管对象 | 它让后台任务、多代理、远程执行能被同一套 UI 和状态系统观察 |
| MCP 连接抽象 | [services/mcp/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/services/mcp/types.ts) | 统一外部 MCP 服务的配置、连接态和序列化表示 | 它让外部能力不是“外挂”，而是系统内的正规公民 |
| Bridge / Remote 会话抽象 | [bridge/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/bridge/types.ts)、[remote/RemoteSessionManager.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/remote/RemoteSessionManager.ts) | 统一远程环境、会话句柄、权限响应和连接配置 | 它让本地 CLI 能演化成可远程调度的平台 |
...
从 [utils/messages.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/utils/messages.ts) 可以看出，系统对消息的工作重点不只是“生成文本”，而是：
...
在 [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) 里，`ToolUseContext` 不是一个轻量参数对象，而是一个“运行时执行容器”。
...
从 [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts) 可见，工具协议不只定义 `call()`，还定义了：
...
[Task.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Task.ts) 里的 `TaskStateBase` 很朴素，但非常关键。它统一了所有任务的共性字段：
...
结合 [tasks/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/types.ts)、[tasks/LocalMainSessionTask.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/LocalMainSessionTask.ts)、[tasks/RemoteAgentTask/RemoteAgentTask.tsx](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/RemoteAgentTask/RemoteAgentTask.tsx)、[tasks/InProcessTeammateTask/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/InProcessTeammateTask/types.ts) 可以看出，派生状态的差异主要来自执行环境不同：
...
[services/mcp/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/services/mcp/types.ts) 先定义了两层配置：
...
从 [bridge/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/bridge/types.ts) 可以看到，`BridgeConfig` 关注的是：
...
在 [remote/RemoteSessionManager.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/remote/RemoteSessionManager.ts) 里，又进一步抽出：
...
从 [tools/AgentTool/AgentTool.tsx](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tools/AgentTool/AgentTool.tsx) 可以看出，AgentTool 不是简单执行一步动作，而是在做这些事：
...
在 [tools/AgentTool/forkSubagent.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tools/AgentTool/forkSubagent.ts) 里，最值得学习的不是实验开关，而是这个思想：
...
[coordinator/coordinatorMode.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/coordinator/coordinatorMode.ts) 进一步说明，多代理协同并不只靠任务状态，还靠：
...
1. 打开 [Tool.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Tool.ts)，试着只看类型定义，不看实现，回答：为什么 `ToolUseContext` 不能只是“工具数组 + 配置”？
2. 对照 [Task.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/Task.ts) 与 [tasks/RemoteAgentTask/RemoteAgentTask.tsx](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/tasks/RemoteAgentTask/RemoteAgentTask.tsx)，写出“公共任务骨架”和“远程任务增量字段”的差异。
3. 对照 [bridge/types.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/bridge/types.ts) 与 [remote/RemoteSessionManager.ts](/Users/zhangyanhua/AI/claude-code-sourcemap-main/restored-src/src/remote/RemoteSessionManager.ts)，解释 `BridgeConfig`、`SessionHandle`、`RemoteSessionConfig` 分别描述的对象是什么。
