---
title: "Claude Code 消息模型：会话上下文如何承载工具回环"
description: "用 ToolUseContext 与 Message 流理解会话态：为何不能把 Agent 当成「发一句收回一句」的无状态聊天系统。"
---
...
你会在 `restored-src/src/query.ts` 里看到一个非常工程化的事实：主循环并不是“一问一答”就结束，而是一个可能反复迭代的 `query` loop。只要 assistant 输出了 `tool_use`，系统就必须执行工具、生成 `tool_result`，再把它们回填进下一轮请求的消息历史中，直到这一轮真正收束。
...
这张图只画本章关心的闭环：用户消息如何进入、上下文如何拼装、assistant 如何返回 `tool_use`、`tool_result` 如何回环进下一轮 `query`。

```mermaid
flowchart TB
  %% Chapter 03: 会话上下文与消息模型（以 query.ts / Tool.ts / toolExecution.ts 为准）

  U["用户输入\n(终端/SDK)"] --> UM["createUserMessage()\n生成 UserMessage"]
  UM --> HIST["messages: Message[]\n(会话历史)"]

  subgraph CTX["会话上下文 ToolUseContext"]
    direction TB
  ...
  Q --> AS["assistant streaming\nyield AssistantMessage"]
  AS -->|content 中包含| TU["tool_use block(s)\n(id/name/input)"]

  TU -->|needsFollowUp=true| RUN["runTools(...) 或 StreamingToolExecutor\n执行工具编排"]
  RUN --> TR["createUserMessage({content:[tool_result]})\n生成 tool_result 的 UserMessage"]
  TR --> HIST2["把 tool_result/attachments\n追加到 messages"]

  HIST2 -->|下一轮| Q

  Q -->|若无 tool_use| DONE["停止继续\n(可能进入 stop hooks / 收尾)"]
```

读图要点：

- **`tool_use` 是 assistant 输出的一部分**，因此消息模型必须能表达它。
- **`tool_result` 被编码为 user 侧内容块**（`type: 'tool_result'` 且带 `tool_use_id`），因此工具执行结果必须以“消息”的形式回填到历史里，而不是仅作为函数返回值留在本地变量。
- **`ToolUseContext.messages` 是“本轮视图”**：`query.ts` 每轮都会把 `messagesForQuery` 写回到 `toolUseContext.messages`，让工具执行、权限判断、hook、附件注入等能力看到同一份上下文。
