---
title: "Claude Code 多代理协同：子任务如何委托与回收"
description: "从 AgentTool 与子任务机制看多代理何时值得：委托、隔离与结果合并，避免把多代理当成「多开几个聊天」。"
---
...
**它不只是一个会调用工具的单线程代理，而是能把任务拆给别的 agent、把这些执行体纳入统一 task 系统，再把结果折回主会话。**
...
- Agent / subagent / teammate 的分工
- 同进程 worker、tmux/iTerm2 worker、远端 worker 的不同执行形态
- 后台运行中的 agent 如何被继续观察、停止、继续发消息
- coordinator 模式怎样把“单 agent”升级成“多 worker 编排”

如果从源码角度看，本章回答的是：

**Claude Code 如何把“派一个 agent 去做事”这句话，落实成任务注册、执行形态分发、进度追踪、通知回流与统一 UI 状态？**
...
```mermaid
flowchart TB
  A["主会话触发 AgentTool"] --> B["解析参数\ndescription / prompt / subagent_type /\nrun_in_background / team_name / name / isolation"]
  B --> C{"team_name + name ?"}
  C -->|是| D["spawnTeammate()"]
  C -->|否| E{"isolation=remote ?"}

  E -->|是| F["teleportToRemote()"]
  F --> G["registerRemoteAgentTask()"]
  G --> H["RemoteAgentTask poll / notify / restore"]

  E -->|否| I{"fork path ?"}
  I -->|是| J["buildForkedMessages()\n继承父上下文"]
  I -->|否| K["普通 agent prompt"]
