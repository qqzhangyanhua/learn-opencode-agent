...
- `provider/models.ts`：描述模型能力、上下文窗口和价格等元信息，并通过 `Data()` / `refresh()` 这类入口维护模型数据，是“为什么能做路由”的前提。
- `session/processor.ts`：在步骤结束时记录 token 与 cost，说明成本不只是配置数据，也会回流成运行时可见信息。
...
<SourceSnapshotCard
  title="第32章源码映射"
  description="性能与成本在 OpenCode 里主要落在模型能力元数据、运行时成本记录、上下文压缩和工具输出裁剪四条主链上。"
