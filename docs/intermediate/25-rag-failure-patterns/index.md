...
<RagAccuracyDemo :scenarios="[
  {
    meta: { id: 'hybrid', label: '混合检索', tone: 'positive' },
    query: '智能体工具调用超时策略是什么？',
    chunks: [
      { id: 'c1', source: 'tool-system.md', text: '工具调用默认超时 30s，可通过 timeout 参数覆盖', score: 0.92, isRelevant: true },
      { id: 'c2', source: 'config.md', text: '全局超时配置位于 agent.config.timeout，单位毫秒', score: 0.85, isRelevant: true },
      { id: 'c3', source: 'errors.md', text: '超时后触发 ToolTimeoutError，可在 onError 钩子处理', score: 0.78, isRelevant: true },
      { id: 'c4', source: 'changelog.md', text: '2024年3月产品版本更新日志概览', score: 0.31, isRelevant: false }
    ],
    expectedAnswer: '工具调用默认超时 30s，可通过 timeout 参数或 agent.config.timeout 全局配置覆盖，超时触发 ToolTimeoutError。',
    evaluation: '混合检索精准匹配关键词「超时」，Precision 75%，Recall 100%，答案完整可信。'
  },
  {
    meta: { id: 'semantic-only', label: '纯语义检索', tone: 'negative' },
    query: '智能体工具调用超时策略是什么？',
    chunks: [
      { id: 'c5', source: 'patterns.md', text: '智能体可以使用多种工具协作完成复杂任务', score: 0.71, isRelevant: false },
      { id: 'c6', source: 'best-practices.md', text: '建议为每个工具设置合理的资源限制', score: 0.68, isRelevant: false },
      { id: 'c7', source: 'config.md', text: '全局超时配置位于 agent.config.timeout，单位毫秒', score: 0.65, isRelevant: true },
      { id: 'c8', source: 'intro.md', text: '工具系统是 Agent 与外部世界交互的核心桥梁', score: 0.61, isRelevant: false }
    ],
    expectedAnswer: '仅找到全局配置，缺少默认值和错误处理，答案不完整。',
    evaluation: '纯语义向量无法区分「超时」关键词，Precision 25%，Recall 33%，高噪声低召回。'
  }
]" />
