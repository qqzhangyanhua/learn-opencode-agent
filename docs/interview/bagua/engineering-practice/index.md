...
**A：** 可能偏好冗长、格式讨好，且与裁判模型强相关。应对：人机混合、多裁判投票、规则评分结
合、对抗样本集。

#### Q12：如何设计 Agent 的回归测试集？

**A：** 覆盖主路径、典型失败、工具错误、长上下文、多语言；每条用例都应包含输入、期望工具或期望答
...
- 上一篇：[← 大模型基础](/interview/bagua/llm-fundamentals/)
- 下一篇：[Prompt 工程 →](/interview/bagua/prompt-engineering/)
- 相关速查：[工程化速查](/interview/engineering/)
- 动手实践：[模型路由实战](/practice/p18-model-routing/)
