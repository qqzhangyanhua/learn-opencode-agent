...
现;广泛用于 LLaMA、ChatGLM 等,外推与相对位置性质较Embedding)
好。
...
序计算且长序列梯度路径长。代价是 (O(n^2)) 注意力内存与时间(相对序列长度)。Q:Decoder 里的 Masked Self-Attention 为什么要 mask?
...
(或显著降低峰值),并融合算子提高吞吐。Q:GQA 相对 MHA 推理上主要省在哪里?
...
多模      GPT-4o 强调语音图像端到端;Gemini 原生多模态与谷歌生态;Claude 长文与代态       码体验常被提及。
