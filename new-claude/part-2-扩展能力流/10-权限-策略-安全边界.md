...
前面几章都在”给 Agent 加能力”。这一章反过来：**给 Agent 加约束**。

为什么 Agent 需要权限系统？因为工具会在真实世界执行动作：删文件、跑命令、改数据库。如果完全信任模型的判断，它可能误删文件或执行危险命令。

Claude Code 对每次工具调用给出三种判定：
...
这个判定还会考虑运行环境：在 CI/自动化模式下没有用户可以确认，所以 `ask` 会退化为 `deny`。

另一个重要机制是**路径安全**（`pathValidation.ts`）：即使规则说”允许”，危险路径（系统目录、工作目录之外）也会被拦截，这是比规则更底层的兜底。

> **源码对应**：`restored-src/src/utils/permissions/permissionSetup.ts`（规则装配）、`restored-src/src/hooks/useCanUseTool.tsx`（运行时决策）。
...
```mermaid
flowchart TB
  RULES["配置规则 + policy + CLI 参数"] --> SETUP["启动期装配\npermissionSetup.ts"]
  SETUP --> CTX["运行时权限上下文\nToolPermissionContext"]
  CTX --> RUN["工具申请执行"]
  RUN --> PATH["路径安全检查\npathValidation.ts"]
  PATH --> CHECK["权限判定\nuseCanUseTool()"]
  CHECK --> ALLOW["allow → 执行"]
  CHECK --> DENY["deny → 拒绝"]
  CHECK --> ASK["ask → 等用户确认"]
  ASK --> RESULT["用户批准或拒绝\n结果回流主循环"]
```
...
1. **启动期把规则装进 context**——运行时不再重新读配置文件
2. **路径检查先于权限判定**——危险路径是比规则更底层的兜底
3. **ask 不等于一定能批准**——自动化模式下 ask 会退化为 deny
