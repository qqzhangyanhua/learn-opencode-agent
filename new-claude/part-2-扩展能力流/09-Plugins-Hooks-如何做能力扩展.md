...
如果 Skill 是”一篇工作手册”，Plugin 就是”一套完整工具箱”——里面可能同时包含手册、工具、操作规程和报警系统。

**Plugin 是能力扩展包**：一个插件安装后，可以一次性向系统注入多种东西，而不是只添加一个功能点。

**Hooks** 是插件体系里另一个关键概念：在特定时机自动触发的脚本，类似”事件监听器”。比如：
- 工具执行**前**：检查参数是否安全
- 工具执行**后**：记录日志
- 会话**结束时**：保存摘要

Hooks 让第三方扩展可以”感知”和”影响”系统行为，而不需要修改主流程代码。

> **源码对应**：`restored-src/src/services/plugins/pluginOperations.ts`（生命周期）、`restored-src/src/utils/plugins/pluginLoader.ts`（加载）。
...
```mermaid
flowchart TB
  SRC[“插件来源\nmarketplace / git / 本地目录”] --> LOADER[“加载与缓存\npluginLoader”]
  LOADER --> EXPAND[“展开能力”]
  EXPAND --> CMDS[“commands / skills → 主命令系统”]
  EXPAND --> HOOKS[“hooks → 工具前后/会话生命周期”]
  EXPAND --> MCP[“MCP servers → 外部工具接入层”]
  EXPAND --> AGENTS[“agents → 子代理能力”]
  OPS[“install / enable / disable / update\npluginOperations”] --> LOADER
```
...
一个插件安装后，可以同时向多个系统层注入能力，而调用方不需要知道这些能力”来自哪个插件”。
