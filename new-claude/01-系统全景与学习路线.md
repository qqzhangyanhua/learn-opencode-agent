...
## 从新手视角看：你的 while 循环会遇到什么

如果你读完了前置知识章节，现在手里有一个 15 行的 while 循环。把它真正跑在生产环境里，你会撞上一连串问题——每一个问题，都对应本书的一个章节：

| 你会遇到的问题 | Claude Code 的解法 | 本书对应章节 |
|---|---|---|
| 消息历史越来越长，API 报”上下文超限” | 多层压缩/裁剪，动态整理消息窗口 | 第 04 章 4.3 节 |
| 模型调了危险命令（如 `rm -rf`），怎么在执行前拦截？ | 权限系统，工具执行前必须通过审批 | 第 05 章、第 10 章 |
| 模型陷入死循环，一直调工具不停 | token budget 系统，系统侧强制决定”该停了” | 第 04 章 4.10 节 |
| 工具 A 和工具 B 能不能同时跑，还是必须排队？ | 并发安全分析，串行/并发两种执行路径 | 第 05 章 4.2 节 |
| 我想接入第三方服务（数据库、浏览器），不想自己写工具实现 | MCP：标准化外部能力接入协议 | 第 07 章 |
| 我想让 Agent 遵循一套固定的工作方法论 | Skills：把方法论注入到上下文 | 第 08 章 |
| 我想让用户通过插件自定义 Agent 行为 | Plugins：可安装的能力包，有完整生命周期 | 第 09 章 |
| 我想在远程服务器上跑 Agent，本地只看结果 | Bridge + Remote Session：远程控制主链路 | 第 11-12 章 |
| 我想同时跑多个 Agent 协同完成复杂任务 | 多代理协同（Agent Tool + Coordinator） | 第 14 章 |

这张表是本书的**学习地图**。每一行都是一个真实的工程问题；每一个章节，都在回答”你自己扩展那个 while 循环时会怎么解决这个问题”。

> **给新手的建议**：不要试图一次读完所有章节。先把第 03-06 章读成一条可复述的主链路（入口 → 消息模型 → 循环 → 工具 → 收尾），再按上面这张表按需选读。
