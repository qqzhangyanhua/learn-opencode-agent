...
想象手术室开始手术前的准备：必须先消毒手术台，再消毒器械，最后做最终确认——顺序不能错，缺一不可。`init.ts` 做的就是类似的事。

当 `claude` 决定进入主会话，它必须在”第一次外发网络请求”之前，把所有基础设施就位：

| 必须先做 | 原因 |
|---|---|
| 加载 TLS 证书 | 首次网络握手前，证书存储可能被运行时缓存 |
| 配置代理 | 预热连接必须走正确的代理，否则绕过了安全出口 |
| 注册退出清理器 | 程序崩溃时遥测和日志才有机会 flush |

这里有一个重要的”分水岭”：**trust（信任建立）**。

- **信任建立前**：系统只允许应用来自安全来源的环境变量（受限 allowlist），因为用户还没确认要信任这个项目。
- **信任建立后**：才能放开完整的项目级环境变量，以及更敏感的遥测初始化。
...
def init():
    # 阶段一：trust 之前（安全基础设施）
    enable_configs()
    apply_safe_env_vars()        # 只放行安全 allowlist
    load_tls_certs()             # 必须在首次网络操作之前
    setup_graceful_shutdown()    # 最早注册，确保退出时能清理

    # 并行发射（不阻塞主链路）
    asyncio.gather(
        init_1p_event_logging(),
        detect_current_repository(),
        populate_oauth_account_info(),
    )

    # 阶段二：trust 之后（用户确认后才做）
    # 主流程会调用 apply_full_env_vars()
    # 然后 init_telemetry_after_trust()
```

> **对 Agent 学习者的意义**：如果你在自己的 Agent 系统里遇到过”遥测乱序”、”配置加载太晚”、”程序退出时日志丢失”这类问题，这一章就是解法参考。初始化顺序不是偶然的——每一步都有其工程理由。
