...
假设你同时开了 3 个 Claude Code 会话：一个处理前端 bug，一个跑测试，一个写文档。

问题来了：**哪个在忙什么？能不能暂停一个？后台跑的退出了能不能恢复？**

`claude ps`、`claude attach`、`claude --bg` 背后就是这一章的内容：**给每个会话建立可观测的”档案”**。

类比：就像操作系统的进程管理器（Activity Monitor / top），Claude Code 需要知道所有正在运行的会话、它们当前在做什么、在等什么。

每个会话启动时会在本地写一个 JSON 文件（`~/.claude/sessions/<pid>.json`），记录：
...
```json
{
  ...
  “lastActivity”: “修改 main.py”
}
```

`claude ps` 读取这些文件，就能显示”哪些会话在跑、在做什么”。

> **源码对应**：`restored-src/src/utils/concurrentSessions.ts`（注册表）、`restored-src/src/entrypoints/cli.tsx`（ps/attach/bg 命令）。
...
```mermaid
flowchart TB
  A["会话启动\nREPL main.tsx"] --> B["注册 session\n写入本地 JSON 文件"]
  B --> C["持续更新状态\nupdateSessionActivity()"]
  C --> D["claude ps 可读取\n显示所有会话状态"]
  A --> E["定期生成 task summary\n记录'在做什么'"]
  E --> D
  A --> F{"退出时"}
  F -->|后台会话| G["tmux detach\n会话继续跑"]
  F -->|前台会话| H["gracefulShutdown\n清理注册表"]
```
